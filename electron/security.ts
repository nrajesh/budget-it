...
export function loadAuthorizedFolders(configPath: string): Set<string> {
  try {
    ...
    if (Array.isArray(config.authorizedFolders)) {
      return new Set(
        config.authorizedFolders.map((f: string) => path.normalize(f)),
      );
    }
    return new Set();
  } catch (error) {
...
export function saveAuthorizedFolders(
  configPath: string,
  folders: Set<string>,
): void {
  try {
...
export function isFolderAuthorized(
  folder: string,
  authorizedFolders: Set<string>,
): boolean {
