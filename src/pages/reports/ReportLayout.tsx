...
const ReportLayout: React.FC<ReportLayoutProps> = ({ title, description, children }) => {
  ...
  const handlePdfExport = async () => {
    try {
      showSuccess("Generating your report... This may take a moment.");
      ...
      if (tables.length === 0) {
        showError("No data tables found in the report to export.");
        return;
      }
      ...
      const { data, error } = await supabase.functions.invoke('create-docling-report', {
      ...
      if (data.download_url) {
        showSuccess("Report created! Your download will begin automatically.");
