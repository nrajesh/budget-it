...
import { Building2, Home, Globe, Baby, Wallet, Landmark, Plus, Upload, RotateCcw } from "lucide-react";
...
const LedgerEntryPage = () => {
    ...
    const filteredLedgers = ledgers.filter(l =>
        ...
        (l.short_name && l.short_name.toLowerCase().includes(searchTerm.toLowerCase()))
    );
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-900 p-4">
            <div className="w-full max-w-2xl space-y-8 animate-in fade-in zoom-in duration-500">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-center justify-center sticky top-2 z-10 bg-gray-50/95 dark:bg-gray-900/95 p-2 rounded-lg backdrop-blur supports-[backdrop-filter]:bg-gray-50/50">
                    ...
                    {selectedLedgers.size > 0 && (
                        <div className="flex items-center gap-2 animate-in fade-in slide-in-from-right-5">
                            <span className="text-sm font-medium text-muted-foreground">{selectedLedgers.size} selected</span>
                            <Button variant="destructive" size="sm" onClick={handleMassDeleteClick}>
                                <Trash2 className="mr-2 h-4 w-4" />
                                Delete Selected
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => setSelectedLedgers(new Set())}>
                                Cancel
                            </Button>
                        </div>
                    )}
                ...
                <div className={filteredLedgers.length === 0 && ledgers.length > 0
                    ...
                    {filteredLedgers.map((ledger) => {
                        ...
                        return (
                            <div key={ledger.id} className="relative group/card">
                                ...
                                <div className={`absolute top-3 right-3 z-20 transition-opacity ${isSelected ? 'opacity-100' : 'opacity-0 group-hover/card:opacity-100'}`}>
                                    <Checkbox
                                        checked={isSelected}
                                        onCheckedChange={(c) => handleToggleSelect(ledger.id, c as boolean)}
                                    ...
                                    <ThemedCardContent>
                                        <p className="text-sm text-muted-foreground">
                                            ...
                                            <span className="block mt-1 text-xs opacity-70">
                                                Last: {ledger.last_accessed ? new Date(ledger.last_accessed).toLocaleDateString() : 'Never'}
                                            </span>
                                        </p>
                                    </ThemedCardContent>
                                </ThemedCard>
