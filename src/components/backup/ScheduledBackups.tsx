...
import { Trash2, FolderOpen, Play, AlertTriangle, ShieldCheck, CheckCircle, XCircle } from "lucide-react";
...
const ScheduledBackups = () => {
    ...
    const [hiddenBackupIds, setHiddenBackupIds] = React.useState<Set<string>>(new Set());

    const parsedFreq = React.useMemo(() => parseFrequency(frequencyInput), [frequencyInput]);
        ...
        try {
            ...
            if (electron) {
                ...
                if (window.showDirectoryPicker) {
                    ...
                    showError("Your browser does not support folder selection for backups.");
                }
            }
        } catch (err: any) {
    ...
    const handleCreateBackup = async () => {
        if (!parsedFreq) {
            showError("Please enter a valid frequency (e.g., '1h', '1 day'). Minimum 1 minute.");
            return;
        }
                ...
                !('showDirectoryPicker' in window) && !getElectronAPI() && (
                    <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-800 dark:border-amber-900/50 dark:bg-amber-950/30 dark:text-amber-200">
                        ...
                        <p className="mt-1 text-sm">
                            Scheduled backups require the File System Access API (Chrome/Edge/Opera) or the Desktop App.
                        </p>
                    </div>
                )
            }

            <Card className={!('showDirectoryPicker' in window) && !getElectronAPI() ? "opacity-60 pointer-events-none" : ""}>
                <CardHeader>
                    ...
                    <CardDescription>
                        ...
                        <span className="text-muted-foreground font-medium flex items-center gap-1 mt-1">
                            ...
                            Backups run automatically in the background while the application is running.
                        </span>
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">
                                Parsed: {parsedFreq ? formatFrequency(parsedFreq) : "Invalid"} (Min: 1m)
                            </p>
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-2">
                                ...
                                >
                                    ...
                                    {backupPath ? backupPath : (dirHandle ? dirHandle.name : "Select Folder")}
                                </Button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-2">
                        <Checkbox
                            id="encrypt"
                            checked={isEncrypted}
                            onCheckedChange={(c: boolean | string) => setIsEncrypted(c === true)}
                            disabled={!('showDirectoryPicker' in window) && !getElectronAPI()}
                        />
                    ...
                    <Button
                        ...
                        disabled={!parsedFreq || (!dirHandle && !backupPath) || (isEncrypted && !password) || (!('showDirectoryPicker' in window) && !getElectronAPI())}
                    >
            ...
            <Card>
                ...
                <CardContent>
                    {displayBackups.length === 0 ? (
                        <div className="text-center py-6 text-muted-foreground">No active backups.</div>
                    ) : (
                        <Table>
                            <TableHeader>
                                <TableRow>
                            ...
                            <TableBody>
                                {displayBackups.map((b: BackupConfig) => (
                                    <TableRow key={b.id}>
                                        ...
                                        <TableCell>
                                            {b.path ? (
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <span className="font-mono text-xs truncate max-w-[150px]" title={b.path}>{b.path}</span>
                                                </div>
                                            ) : (
                                                <TooltipProvider>
                                                    <Tooltip>
                                                        <TooltipTrigger asChild>
                                                            <div className="flex items-center gap-1 cursor-help underline decoration-dotted underline-offset-4 decoration-muted-foreground/30">
                                                                <FolderOpen className="h-3 w-3 text-muted-foreground" />
                                                                <span className="truncate max-w-[120px]">{b.directoryHandle?.name || "Unknown"}</span>
                                                            </div>
                                                        </TooltipTrigger>
                                                        <TooltipContent>
                                                            <p className="font-medium">{b.directoryHandle?.name}</p>
                                                            <p className="text-xs text-muted-foreground mt-1">Full path is hidden by browser security.</p>
                                                        </TooltipContent>
                                                    </Tooltip>
                                                </TooltipProvider>
                                            )}
                                        ...
                                        <TableCell>
                                            {b.encrypted ? <Badge variant="default">Encrypted</Badge> : <Badge variant="secondary">Plain</Badge>}
                                        </TableCell>
                                                ...
                                                <Tooltip>
                                                    ...
                                                    <TooltipContent>
                                                        {b.isActive ? (
                                                            ...
                                                            <p>Backup disabled. Info: Folder missing or permission denied.</p>
                                                        )}
                                                    </TooltipContent>
                                                </Tooltip>
                                            </TooltipProvider>
                                        ...
                                        <TableCell>
                                            {b.isActive ? (
                                                <>
                                                    ...
                                                    {b.lastBackup && <div className="text-xs text-muted-foreground">Last: {new Date(b.lastBackup).toLocaleTimeString()}</div>}
                                                </>
                                            ) : (
                                        ...
                                        <TableCell className="text-right">
                                            {!b.path && (
                                                <Button variant="ghost" size="icon" onClick={() => verifyPermission(b)} className="text-amber-500 hover:text-amber-600" title="Verify/Grant Permission">
                                                    <ShieldCheck className="h-4 w-4" />
                                                </Button>
                                            )}
                                            <Button variant="ghost" size="icon" onClick={() => handleDelete(b.id)} className="text-red-500 hover:text-red-600" title="Delete Schedule">
