...
const NetWorthStatement: React.FC<NetWorthStatementProps> = ({ transactions: _filteredTransactions, allTransactions, accounts }) => {
  ...
  const { assets, liabilities, netWorth, isProjected } = React.useMemo(() => {
    ...
    return {
      ...
      isProjected: allTransactions.some(t => t.is_scheduled_origin && validAccountNames.has(t.account) && new Date(t.date) <= cutoffDate)
    };
  }, [allTransactions, accounts, selectedCurrency, convertBetweenCurrencies, dateRange]);
    ...
    <ThemedCard>
      ...
      <ThemedCardContent>
        <div className="grid gap-6 md:grid-cols-3">
          ...
          <ThemedCard>
            <ThemedCardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <ThemedCardTitle className="text-sm font-medium">Net Worth {isProjected && <span className="text-muted-foreground text-xs font-normal">(Projected)</span>}</ThemedCardTitle>
