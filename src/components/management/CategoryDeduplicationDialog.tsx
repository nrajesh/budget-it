...
const CategoryDeduplicationDialog: React.FC<
...
> = ({ isOpen, onClose }) => {
  ...
  const handleMerge = async () => {
    ...
    try {
      await dataProvider.mergeCategories(
        selectedMaster,
        selectedDuplicates,
        activeLedger?.id || "",
      );
      showSuccess(
        `Successfully merged ${selectedDuplicates.length} categories into ${selectedMaster}.`,
      );
      await invalidateAllData();
      onClose();
    } catch (error: unknown) {
      showError(`Merge failed: ${error instanceof Error ? error.message : String(error)}`);
    } finally {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Select the primary category you want to keep, then select the
            duplicate categories to merge into it. All transactions from
            duplicates will be moved to the primary category.
          </DialogDescription>
        </DialogHeader>
          ...
          {selectedMaster && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Selected duplicates will be permanently deleted after merging
                their transactions.
              </p>
            </div>
          )}
        ...
        <DialogFooter>
          ...
          <Button
            onClick={handleMerge}
            disabled={
              isProcessing || !selectedMaster || selectedDuplicates.length === 0
            }
          >
