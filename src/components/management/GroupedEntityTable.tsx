...
import { Edit, Trash2, Loader2, ChevronDown, ChevronRight, ChevronLeft, ChevronRight as ChevronRightIcon } from "lucide-react";
import {
    ContextMenu,
    ContextMenuContent,
    ContextMenuItem,
...
export const GroupedEntityTable = <T extends { id: string; name: string }>({
    ...
    const groupKeys = React.useMemo(() => Object.keys(groupedData).sort(), [groupedData]);
                ...
                <TableBody>
                    {isLoading ? (
                        ...
                        groupKeys.map((groupKey) => {
                            ...
                            const totalPages = Math.ceil(groupItems.length / ITEMS_PER_GROUP_PAGE);

                            const startIndex = (currentPage - 1) * ITEMS_PER_GROUP_PAGE;
                            const pagedItems = groupItems.slice(startIndex, startIndex + ITEMS_PER_GROUP_PAGE);
                                    ...
                                    >
                                        <TableCell colSpan={columns.length + 2} className="font-semibold py-2">
                                            <div className="flex items-center space-x-2">
                                                ...
                                                <span>{groupKey}</span>
                                                <span className="text-xs text-muted-foreground font-normal ml-2">({groupItems.length} accounts)</span>
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                                    ...
                                                    <TableCell colSpan={columns.length + 2} className="py-2">
                                                        <div className="flex items-center justify-center space-x-4">
                                                            ...
                                                            <Button
                                                                ...
                                                                onClick={(e) => {
                                                                    e.stopPropagation();
                                                                    setPage(groupKey, Math.min(totalPages, currentPage + 1));
                                                                }}
