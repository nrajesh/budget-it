        ...
        activeMonthlyBudgets.forEach(b => {
            ...
            const spentForBudget = transactions
                .filter(t => {
                    ...
                    return t.category === b.category_name &&
    ...
    if (metrics.totalBudget === 0 && metrics.totalSpent === 0) {
        return (
            <Card className="h-full">
                ...
                <CardContent>
                    <p className="text-xs text-muted-foreground">No active budgets setup.</p>
                </CardContent>
            </Card>
        );
    }
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex justify-between items-end">
                        ...
                        <div className="text-right">
                            <p className={`text-2xl font-bold ${isOverBudget ? 'text-red-500' : 'text-foreground'}`}>
                                {formatCurrency(metrics.totalSpent)}
                            </p>
                            <p className="text-xs text-muted-foreground">Spent {metrics.percentage > 0 && `(${metrics.percentage.toFixed(0)}%)`}</p>
                        </div>
                    </div>
                    {/* Progress Bar */}
