...
export const BudgetHealthWidget = () => {
  ...
  const healthData = React.useMemo(() => {
    ...
    const activeMonthlyBudgets = budgets.filter((b) => {
      ...
      return (
        b.frequency === "1m" && startDate <= now && (!endDate || endDate >= now)
      );
    });
    ...
    const totalBudget = activeMonthlyBudgets.reduce((sum, b) => {
      return (
        sum +
        convertBetweenCurrencies(b.target_amount, b.currency, selectedCurrency)
      );
    }, 0);
    ...
    const totalSpending = transactions
      .filter((t) => {
        ...
        const budgetForTransaction = activeMonthlyBudgets.find(
          (b) => b.category_name === t.category,
        );
        return (
          budgetForTransaction &&
      ...
      <Card>
        ...
        <CardContent className="flex items-center justify-center h-24">
          <p className="text-sm text-muted-foreground">
            No active monthly budgets for this period.
          </p>
        </CardContent>
      </Card>
    );
  ...
  const progressColor =
    healthData.percentage > 100 ? "bg-destructive" : "bg-primary";
    ...
    <Card>
      ...
      <CardContent>
        <div className="space-y-2">
          ...
          <Progress
            value={Math.min(healthData.percentage, 100)}
