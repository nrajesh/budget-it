...
export const SearchFilterBar = () => {
    ...
    const { categories, subCategories, accounts, vendors, transactions } = useTransactions();
    ...
    const handleConversationalUpdate = (update: Partial<ParsedFilterState>) => {
        ...
        if (update.selectedCategories) {
            // Merge unique
            const next = Array.from(new Set([...selectedCategories, ...update.selectedCategories]));
            setSelectedCategories(next);

            // If "Transfer" category is selected (likely slug 'transfer' or 'transfers'), ensure we don't exclude them
            const hasTransfer = next.some(s => s.toLowerCase().includes('transfer'));
            if (hasTransfer) {
        ...
        if (update.selectedSubCategories) {
            const next = Array.from(new Set([...selectedSubCategories, ...update.selectedSubCategories]));
        ...
        if (update.selectedAccounts !== undefined) {
            ...
            // Let's merge for now.
            const next = Array.from(new Set([...(selectedAccounts || []), ...update.selectedAccounts]));
            setSelectedAccounts(next);
        }

        if (update.selectedVendors) {
            const next = Array.from(new Set([...selectedVendors, ...update.selectedVendors]));
