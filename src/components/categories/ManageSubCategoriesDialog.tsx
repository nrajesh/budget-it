...
const ManageSubCategoriesDialog: React.FC<ManageSubCategoriesDialogProps> = ({
    ...
    const isLoading = addSubCategoryMutation.isPending || renameSubCategoryMutation.isPending || deleteSubCategoryMutation.isPending;
            ...
            <Dialog open={isOpen} onOpenChange={onOpenChange}>
                <DialogContent className="max-w-md max-h-[80vh] flex flex-col">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            Manage sub-categories for <span className="font-semibold text-primary">{category?.name}</span>.
                    ...
                    <div className="flex gap-2 my-2">
                        <Input
                            ...
                            disabled={isLoading}
                        />
                        <Button onClick={handleAddSubCategory} disabled={!newSubCategoryName.trim() || isLoading}>
                                ...
                                {subCategories.length === 0 ? (
                                    <TableRow>
                                        <TableCell colSpan={3} className="text-center text-muted-foreground py-8">
                                            No sub-categories found for this category. Use the input above to add one.
                                        </TableCell>
                                    </TableRow>
                                ) : (
                                            ...
                                            <TableCell className="text-right">
                                                {editingSubCategory === sub ? (
                                                    <div className="flex justify-end gap-1">
                                                        <Button size="icon" variant="ghost" className="h-8 w-8 text-green-600" onClick={() => handleSaveEdit(sub)} disabled={isLoading}>
                                                            <Save className="h-4 w-4" />
                                                        </Button>
                                                        <Button size="icon" variant="ghost" className="h-8 w-8 text-muted-foreground" onClick={handleCancelEdit} disabled={isLoading}>
                                                            <X className="h-4 w-4" />
                                                        </Button>
                                                    </div>
                                                ) : (
                                                    <div className="flex justify-end gap-1">
                                                        <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => handleStartEdit(sub)} disabled={isLoading}>
                                                            <Edit className="h-4 w-4" />
                                                        </Button>
                                                        <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => handleDeleteClick(sub)} disabled={isLoading}>
