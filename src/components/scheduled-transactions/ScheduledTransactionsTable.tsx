...
export function ScheduledTransactionsTable({ transactions = [], onEdit, onDelete, onBulkDelete, onProcessToday, onUnlink }: ScheduledTransactionsTableProps) {
    ...
    const toggleSelect = (id: string) => {
        ...
        if (txn?.transfer_id) {
            const pair = transactions.find(t => t.transfer_id === txn.transfer_id && t.id !== id);
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            ...
                            <TableHead className="text-slate-800 dark:text-slate-200 font-semibold">Next Date</TableHead>
                            <TableHead className="text-slate-800 dark:text-slate-200 font-semibold">Frequency</TableHead>
                    ...
                    <TableBody>
                        {transactions.length === 0 ? (
                            ...
                            currentTransactions.map((transaction) => {
                                ...
                                const currentAccountCurrency = accountCurrencyMap.get(transaction.account) || transaction.currency || 'USD';
                                ...
                                return (
                                    <ContextMenu key={transaction.id || Math.random()}>
                                            ...
                                            >
                                                ...
                                                <TableCell>
                                                    <div className="flex items-center">
                                                        ...
                                                        <span onClick={(e) => { e.stopPropagation(); handleVendorClick(transaction.vendor); }} className="font-medium cursor-pointer hover:text-primary hover:underline text-slate-700 dark:text-slate-200">
                                                            {transaction.vendor || 'Unknown'}
                                                        </span>
                                                    </div>
                                                    <div onClick={(e) => { e.stopPropagation(); handleAccountClick(transaction.account); }} className="text-sm text-slate-500 dark:text-slate-400 cursor-pointer hover:text-primary hover:underline">{transaction.account || 'Unknown'}</div>
                                                </TableCell>
                                                <TableCell>
                                                    <Badge variant="outline" onClick={(e) => { e.stopPropagation(); handleCategoryClick(transaction.category); }} className={`text-slate-700 dark:text-slate-300 ${transaction.category !== 'Transfer' ? "cursor-pointer hover:border-primary" : ""}`}>
                                                ...
                                                <TableCell>
                                                    <DropdownMenu>
                                                        ...
                                                        <DropdownMenuContent align="end">
                                                            ...
                                                            {onProcessToday && (
                                                                <DropdownMenuItem onClick={() => onProcessToday(transaction)}>
                                                                    <PlayCircle className="mr-2 h-4 w-4" /> Make happen today
                                                                </DropdownMenuItem>
                                                            )}
                                        ...
                                        <ContextMenuContent>
                                            ...
                                            <ContextMenuSeparator />
                                            {onProcessToday && (
                                                <ContextMenuItem onClick={() => onProcessToday(transaction)}>
                                                    <PlayCircle className="mr-2 h-4 w-4" /> Make happen today
                                                </ContextMenuItem>
                                            )}
                                            <ContextMenuSeparator />
                                            {onEdit && <ContextMenuItem onClick={() => onEdit(transaction)}><Pencil className="mr-2 h-4 w-4" /> Edit</ContextMenuItem>}
                                            {onDelete && <ContextMenuItem onClick={() => onDelete(transaction.id)} className="text-destructive"><Trash className="mr-2 h-4 w-4" /> Delete</ContextMenuItem>}
                                        </ContextMenuContent>
                                    </ContextMenu>
                                );
                            })
