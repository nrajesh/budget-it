...
export const useContinuitySync = () => {
    ...
    useEffect(() => {
        ...
        if (!config.autoSyncEnabled || !config.syncDirectoryHandle || needsPermission) return;
            ...
            try {
                ...
                if (fileContent) {
                    ...
                    if (importResult.type === "success") {
                        ...
                        toast.error("Auto-import failed: Sync file is encrypted. Manual import required.");
                    }
                }
            } catch (err: unknown) {
                if (err instanceof Error && err.name === "NotFoundError") {
                    // It's okay if the file doesn't exist yet, it means this is the first sync
                    console.log("No ledger.json found in sync directory. A new one will be created on next change.");
                } else {
                    console.error("Failed to auto-import sync data:", err);
                    setSyncError(err instanceof Error ? err.message : "Unknown error during sync import");
        ...
        performBootImport();
    }, [isReady, config, needsPermission, dataProvider, switchLedger, updateLastSync]);
    ...
    const triggerExport = async () => {
        ...
        try {
            const payload: SyncPayloadEnvelope = await generateBackupData(dataProvider);
            ...
            setSyncError(err instanceof Error ? err.message : "Unknown error during sync export");
