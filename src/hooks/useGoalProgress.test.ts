...
describe("calculateGoalProgress", () => {
    ...
    it("sums matching transactions after start date", () => {
        const budget = makeBudget({ category_name: "Savings", budget_scope: "category" });
        const txs = [
            makeTx({ id: "t1", amount: 200, category: "Savings", date: "2026-01-10T00:00:00.000Z" }),
            makeTx({ id: "t2", amount: 300, category: "Savings", date: "2026-01-20T00:00:00.000Z" }),
        ];
    ...
    it("ignores transactions before start date", () => {
        ...
        const txs = [
            makeTx({ id: "t1", amount: 500, category: "Savings", date: "2026-01-15T00:00:00.000Z" }),
        ];
        const result = calculateGoalProgress(budget, txs, []);
        expect(result.accumulatedAmount).toBe(0);
    });
    ...
    it("filters by account scope", () => {
        const budget = makeBudget({ budget_scope: "account", budget_scope_name: "My Savings" });
        const txs = [
            makeTx({ id: "t1", amount: 100, account: "My Savings", category: "Other" }),
    ...
    it("filters by sub-category scope", () => {
        const budget = makeBudget({ budget_scope: "sub_category", budget_scope_name: "Vacation Fund" });
        const txs = [
            makeTx({ id: "t1", amount: 150, category: "Other", sub_category: "Vacation Fund" }),
            makeTx({ id: "t2", amount: 200, category: "Other", sub_category: "Emergency" }),
        ];
        const result = calculateGoalProgress(budget, txs, []);
