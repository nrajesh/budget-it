...
export const useTransactionPairing = (
  transactions: Transaction[],
): UseTransactionPairingResult => {
  return useMemo(() => {
        ...
        credits.forEach((credit) => {
          ...
          for (const debit of debits) {
            ...
            if (
              credit.sub_category &&
              debit.sub_category &&
              credit.sub_category === debit.sub_category
            )
              score += 1;

            // Same Vendor?
            if (credit.vendor && debit.vendor && credit.vendor === debit.vendor)
              score += 1;
