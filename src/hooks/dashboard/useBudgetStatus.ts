...
import { subDays, format, isAfter, isBefore, endOfDay, addMonths, addYears, parse } from "date-fns";
...
export const useBudgetStatus = () => {
    ...
    const budgetStats = useMemo(() => {
        if (!budgets.length) return { totalBudget: 0, totalSpent: 0, percentage: 0, hasBudgets: false, dailyTrends: [] };

        // Filter active and spending budgets (ignore goals for now?)
        // Usually "Budget Status" implies spending limits.
        const spendingBudgets = budgets.filter((b) => !b.is_goal && b.is_active !== false);

        if (!spendingBudgets.length) return { totalBudget: 0, totalSpent: 0, percentage: 0, hasBudgets: false, dailyTrends: [] };
        ...
        spendingBudgets.forEach(b => {
            if (b.category_name) budgetedCategories.add(b.category_name.toLowerCase());
        });
        ...
        const dailyMap = new Map<string, { spending: number; replenishment: number }>();
        ...
        transactions.forEach(t => {
            ...
            if (isAfter(tDate, thirtyDaysAgo) && isBefore(tDate, endOfDay(today))) {
                if (t.amount < 0 && t.category && budgetedCategories.has(t.category.trim().toLowerCase())) {
        ...
        spendingBudgets.forEach(budget => {
            ...
            while (isBefore(currentDate, endOfDay(today)) && loopLimit < 1000) {
                ...
                if (isAfter(currentDate, thirtyDaysAgo)) {
                    ...
                    if (dailyMap.has(dateKey)) {
                        const entry = dailyMap.get(dateKey)!;
                        const amount = convertBetweenCurrencies(budget.target_amount, budget.currency, selectedCurrency || "USD");
                ...
                switch (budget.frequency) {
                    ...
                    default: loopLimit = 1001; break; // One-time or unknown
                }
                loopLimit++;
            }
        });
    ...
    }, [budgets, transactions, accounts, vendors, convertBetweenCurrencies, selectedCurrency]);
