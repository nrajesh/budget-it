...
export const useFinancialRunway = () => {
    ...
    const monthlyFixedFlow = useMemo(() => {
        ...
        scheduledTransactions.forEach((t) => {
            ...
            if (freq.includes("weekly")) monthlyMultiplier = 4.33; // Average weeks in a month
    ...
    const monthlyDiscretionaryFlow = useMemo(() => {
        ...
        return { income: avgIncome, expenses: avgExpenses, net: avgIncome - avgExpenses };
    }, [transactions, selectedCurrency, convertBetweenCurrencies]);


    // 4. Combined Burn & Runway
    const runwayData = useMemo(() => {
        const totalMonthlyIncome = monthlyFixedFlow.income + monthlyDiscretionaryFlow.income;
        const totalMonthlyExpenses = monthlyFixedFlow.expenses + monthlyDiscretionaryFlow.expenses;
