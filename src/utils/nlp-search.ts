...
export const parseSearchQuery = (query: string): SearchFilters => {
    ...
    if (!filters.dateRange) {
        ...
        if (yearOnlyMatch) {
            ...
            filters.dateRange = { from: new Date(year, 0, 1), to: new Date(year, 11, 31) };
        }
    }
    ...
    ['last week', 'this week', 'last month', 'this month', 'last year', 'this year', 'yesterday', 'today', 'negative'].forEach(k => {
        residual = residual.replace(k, "");
    });

    // Remove months
    ['january', 'february', 'march', 'april', 'may', 'june', 'july', 'august', 'september', 'october', 'november', 'december'].forEach(k => {
        residual = residual.replace(k, "");
    });
    ['jan', 'feb', 'mar', 'apr', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec'].forEach(k => { // may is covered
    ...
    // Clean up whitespace
    const terms = residual.split(/\s+/).filter(t => t.length > 0 && t !== '>' && t !== '<' && t !== '..');
    if (terms.length > 0) {
        filters.text = terms;
    }
...
export const filterTransactions = (transactions: any[], query: string) => {
    ...
    return transactions.filter(t => {
        ...
        if (filters.text && filters.text.length > 0) {
            const searchableText = `${t.vendor} ${t.category} ${t.account} ${t.remarks || ''}`.toLowerCase();
            // All terms must match (AND logic)
            const textMatch = filters.text.every(term => searchableText.includes(term));
...
export const filterAccounts = (accounts: any[], query: string) => {
    ...
    return accounts.filter(acc => {
        ...
        // Text Check
        if (filters.text && filters.text.length > 0) {
            const searchableText = `${acc.name} ${acc.currency} ${acc.remarks || ''}`.toLowerCase();
            const textMatch = filters.text.every(term => searchableText.includes(term));
            if (!textMatch) match = false;
        }
