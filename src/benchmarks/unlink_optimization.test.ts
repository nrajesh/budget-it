...
describe("Unlink Transactions Optimization Benchmark", () => {
  ...
  it("Baseline: Inefficient Loop Unlink", async () => {
    ...
    console.log(
      `[Baseline] Setup complete. Starting unlink of ${TRANSACTION_COUNT} items...`,
    );
    ...
    console.log(
      `[Baseline] Loop Unlink (${TRANSACTION_COUNT} items): ${(end - start).toFixed(2)}ms`,
    );
  ...
  it("Optimized: Bulk Modify Unlink", async () => {
    ...
    console.log(
      `[Optimized] Setup complete. Starting unlink of ${TRANSACTION_COUNT} items...`,
    );
    const start = performance.now();
    ...
    const end = performance.now();
    console.log(
      `[Optimized] Modify Unlink (${TRANSACTION_COUNT} items): ${(end - start).toFixed(2)}ms`,
    );
